...
const App: React.FC = () => {
  ...
  }, [currentSlide]); // Added dependency to ensure state is fresh
      ...
      <main className="no-print h-screen w-screen bg-slate-50 flex flex-col font-sans selection:bg-purple-100 selection:text-purple-900 overflow-hidden relative">
        ...
        <header className="w-full px-6 md:px-12 py-6 z-50 flex justify-between items-center bg-transparent relative">
            ...
            <button 
              ...
              className="p-2 rounded-full bg-white/50 backdrop-blur-sm border border-slate-200 text-slate-500 hover:bg-white hover:text-purple-600 hover:border-purple-200 transition-all shadow-sm group cursor-none"
        ...
        <footer className="w-full px-6 md:px-12 py-6 z-50 flex items-end justify-between bg-transparent relative">
          ...
          {/* Progress Bar (Clickable for Quick Nav) */}
          <div className="absolute bottom-0 left-0 w-full h-1 bg-slate-200/50 flex">
              {SLIDES.map((_, idx) => (
                  <div 
                    key={idx}
                    onClick={() => goToSlide(idx)}
                    className="h-full flex-1 cursor-pointer hover:bg-purple-200 transition-colors relative group"
                  >
                     {/* Tooltip on hover */}
                     <div className="absolute bottom-2 left-1/2 -translate-x-1/2 bg-slate-900 text-white text-[9px] px-2 py-1 rounded opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap pointer-events-none mb-1">
                        {SLIDES[idx].title}
                     </div>
                  </div>
              ))}
          ...
          <div className="flex items-center gap-3 animate-enter delay-300 ml-auto mb-2">
              ...
              <button 
                  ...
                  className="group relative w-12 h-12 rounded-full flex items-center justify-center bg-slate-900 border border-slate-900 text-white hover:bg-purple-600 hover:border-purple-600 disabled:opacity-30 disabled:cursor-not-allowed transition-all shadow-lg hover:shadow-purple-200 hover:scale-105 active:scale-95 cursor-none"
              >
