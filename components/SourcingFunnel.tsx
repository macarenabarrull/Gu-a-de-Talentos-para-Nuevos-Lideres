...
export const SourcingFunnel: React.FC = () => {
  return (
    <section className="h-full flex flex-col justify-start md:justify-center max-w-7xl mx-auto w-full px-4 pt-8 md:pt-0">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 lg:gap-8 mt-6 lg:mt-10 flex-1 min-h-0 items-start">
         {SOURCING_CHANNELS.map((channel, index) => {
            ...
            return (
              ...
              >
                 ...
                 <div className="absolute -right-4 -top-6 text-[10rem] font-brand font-black text-slate-100 opacity-50 group-hover:text-slate-200 group-hover:opacity-80 transition-colors pointer-events-none select-none">
                     {index + 1}
                 </div>

                 <div className="relative z-10 flex flex-col h-full">
                    <div className={`w-16 h-16 rounded-2xl ${colors.iconBg} ${colors.iconColor} flex items-center justify-center mb-6 group-hover:scale-110 transition-transform duration-300 shadow-sm`}>
                       <channel.icon className="w-8 h-8" />
                    </div>
                    
                    <h3 className="font-brand font-black text-2xl text-slate-900 mb-4 leading-tight">
                        {channel.title}
                    </h3>
                    ...
                    <div className={`mt-auto py-2 px-4 rounded-xl ${colors.bg} ${colors.iconColor} text-xs font-bold uppercase tracking-wide inline-block self-start border border-black/5`}>
                        {channel.percentage}
                    </div>
                 </div>
              </div>
      ...
      <div className="mt-8 lg:mt-12 bg-white/60 backdrop-blur-xl p-6 lg:p-8 rounded-2xl border border-white shadow-lg text-center animate-enter delay-500 shrink-0 mb-4 max-w-3xl mx-auto">
        <p className="text-slate-700 text-sm lg:text-base font-medium">
